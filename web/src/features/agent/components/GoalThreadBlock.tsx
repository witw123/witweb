  ...
  return (
    <div>
      ...
      >
        ...
        <div className="flex items-center gap-2">
          {outputSummary && (
            <span className="text-xs text-zinc-500 truncate max-w-[120px]">{outputSummary}</span>
          )}
          <span className="text-zinc-500 text-xs">{isExpanded ? "收起" : "详情"}</span>
        </div>
      </button>

      {/* 审批卡片 */}
      {pendingApprovals.length > 0 && isExpanded && (

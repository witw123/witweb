...
export function listBlogs(page = 1, limit = 20, search = "", status = "", usernameFilter = "", sort = "created_at_desc") {
  ...
  const total = (db.prepare(`SELECT COUNT(*) AS cnt FROM posts p ${where}`).get(...params) as any)?.cnt || 0;
...
export function listCategories(page = 1, limit = 100, search = "") {
  const db = getBlogDb();
  const offset = (page - 1) * limit;
  let where = "WHERE 1=1";
  const params: any[] = [];
  ...
  const total = (db.prepare(`SELECT COUNT(*) AS cnt FROM categories c ${where}`).get(...params) as any)?.cnt || 0;
...
export function createCategory(data: { name: string; slug: string; description?: string; is_active?: number }) {
  const db = getBlogDb();
  const maxSort = (db.prepare("SELECT COALESCE(MAX(sort_order), -1) AS max_sort FROM categories").get() as any)?.max_sort ?? -1;
...
export function updateCategory(id: number, data: { name?: string; slug?: string; description?: string; is_active?: number }) {
  const db = getBlogDb();
  const fields: string[] = [];
  const params: any[] = [];

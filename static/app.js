...
const CHARACTER_REF_RE = /^[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+$/;
...
function parseCharacterRefs(input) {
  ...
  const invalid = tokens.filter(t => !CHARACTER_REF_RE.test(t));
  if (invalid.length) {
    log(`角色 ID 格式错误: ${invalid.join(", ")}`);
    return null;
  }
...
function insertCharacterRefToPrompt(ref) {
  ...
  if (!promptEl) return;
  const tag = `@${ref}`;
  if (!promptEl.value.includes(tag)) {
    promptEl.value = `${tag} ${promptEl.value}`.trim();
  }
}
...
window.onload = () => {
  ...
  fetch("/config").then(r => r.json()).then(j => {
    ...
    const refs = j.query_defaults && Array.isArray(j.query_defaults.character_refs)
      ...
      : [];
    recentCharacterRefs = refs.slice(0, 8);
    renderCharacterRefList();
    getAccountCredits();
  });
